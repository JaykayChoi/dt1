...
{
    /// <summary>
    /// 레일 엣지를 용량 1의 점유 구간으로 관리하는 교통 제어기.
    /// SimResource의 Request/Release 콜백 패턴을 엣지 단위로 적용하되,
    /// 데드락 탐지를 위해 점유자·대기자 관계를 겉으로 드러낸다.
    /// </summary>
    public sealed class SegmentController
    {
        ...
        public IReadOnlyList<int> DetectDeadlockCycle()
        {
            ...
            var state = new Dictionary<int, int>();  // 0 미방문, 1 방문중, 2 완료
            foreach (int start in next.Keys)
            {
                ...
                while (true)
                {
                    if (!next.TryGetValue(node, out int _))
                    {
                        // 막다른 길 — 이 경로의 노드들을 완료 처리.
                    ...
                    {
                        // 방문 중인 노드를 다시 만났다 = 사이클. stack에서 그 노드부터가 사이클.
